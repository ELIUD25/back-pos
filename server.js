...
const createModels = () => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      throw new Error('Database not connected. Models cannot be created.');
    }
    ...
    // Return fallback models that won't crash the app
    return createFallbackModels();
  }
};

// Fallback models to prevent crashes
const createFallbackModels = () => {
  console.log('⚠️ Creating fallback models...');
  const fallbackModel = {
    ...
    create: () => Promise.reject(new Error('Models not initialized')),
    deleteOne: () => Promise.reject(new Error('Models not initialized'))
  };

  return {
    Product: fallbackModel,
    Shop: fallbackModel,
    Cashier: fallbackModel,
    Expense: fallbackModel,
    Transaction: fallbackModel,
    Credit: fallbackModel,
    User: fallbackModel,
    SecureCode: fallbackModel
  };
};

// Initialize with fallback models first
models = createFallbackModels();
...
const connectDB = async () => {
  try {
    ...
    // Reinitialize models with proper database connection
    models = createModels();
    await initializeEmail();
    await createDefaultAdmin();
    ...
    console.error('❌ MongoDB connection failed:', error.message);
    // Keep fallback models to prevent crashes
    console.log('⚠️ Using fallback models - some features may be limited');
  }
};
...
const checkDBConnection = (req, res, next) => {
  ...
  // Check if models are properly initialized (not fallback)
  if (!models.User || typeof models.User.create === 'function' && models.User.create.toString().includes('Models not initialized')) {
    return res.status(503).json({
      success: false,
...
// Request secure login code
app.post('/api/auth/request-code',
  checkDBConnection, // Add database health check
  [
    body('email').isEmail().normalizeEmail()
  ],
  async (req, res) => {
    try {
      ...
      try {
        ...
        return res.status(503).json({
          success: false,
          message: 'Failed to save secure code. Please try again.'
        });
      }

      if (!emailTransporter) {
        console.log('📧 Development mode - secure code:', secureCode);
        return res.json({
          success: true,
          message: 'Secure code generated (email service disabled)',
          developmentMode: true,
      ...
      try {
        ...
        try {
          ...
          console.error('❌ Failed to delete secure code:', deleteError);
        }
        res.status(500).json({
          success: false,
          message: 'Failed to send secure code. Please try again later.'
...
app.post('/api/auth/verify-code',
  ...
  async (req, res) => {
    try {
      ...
      // Find the secure code with error handling
      let secureCode;
      try {
      ...
      if (secureCode.attempts >= 5) {
        try {
          ...
          console.error('❌ Error deleting over-attempted code:', deleteError);
        }
        return res.status(400).json({
          success: false,
          message: 'Too many failed attempts. Please request a new code.'
      ...
      if (!isValidCode) {
        try {
          ...
          console.error('❌ Error updating attempt count:', saveError);
        }
...
// ... [Keep the rest of your existing code for calculations, utilities, etc.]

// ==================== HEALTH CHECK ENDPOINT ====================

app.get('/api/health', (req, res) => {
  const dbStatus = mongoose.connection.readyState === 1 ? 'connected' : 'disconnected';
  const modelsStatus = models.User && !models.User.create.toString().includes('Models not initialized') ? 'initialized' : 'fallback';
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(`🔧 Models: ${models.User ? 'Initialized' : 'Initializing...'}`);
      console.log('='.repeat(60));
    });

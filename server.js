...
// Enhanced CORS configuration - handle preflight requests first
app.options('*', cors({
  origin: [
...
const connectDB = async () => {
  try {
    const connectionString = process.env.MONGODB_URI || 'mongodb+srv://chemistseridah_db_user:m5pBLBogNk9Ov714@cluster0.5pw7hqj.mongodb.net/chemistseridah?retryWrites=true&w=majority';
...
const initializeEmail = async () => {
  try {
    ...
    if (emailTransporter) {
      ...
      console.log('⚠️ Email transporter not configured - running in development mode');
      return false;
    }
  } catch (error) {
    console.error('❌ Email configuration error:', error.message);
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(`📧 Email: ${emailTransporter ? 'configured' : 'development mode'}`);
      console.log('='.repeat(50));
    });
